...
Shader "Noise/Mobile/Advanced Simplex/Badlands" 
{
	Properties 
	{
		...
		_Floor ("Floor", Float) = 2.0
		_RidgePower("Ridge Power", Range(1.0, 8.0)) = 1.2
		_RidgeOffset("Ridge Offset", Float) = 0.75
	...
	SubShader 
	{
		Pass
		{
			...
			{
				return offset - abs(4.0*SimplexPerlin3D_Deriv(p));
			}

			//
			// 3D fractional Brownian Motion
			//
			float fBM(vec3 p, float floorV, vec3 offset, float frequency, float amplitude, float lacunarity, float persistence, float ridgePower, float ridgeOffset, float warp)
			{
				float sum = floorV;
				float bias = 0.0;
				...
				{
					vec4 n = 0.25 * ridge((p+offset+warp*dsum)*frequency, ridgePower, ridgeOffset);
            ...
            {	
				...
				if(_Displace > 0.0)
				{
					position.y = fBM(vec3(position.xz, _Time*_AnimSpeed), _Floor, _NoiseOffset, _Frequency, _Amplitude, _Lacunarity, _Persistence, _RidgePower, _RidgeOffset, _Warp);
				}
            ...
            void main()
            {
				float h = fBM(vec3(vec2(position.xz), _Time*_AnimSpeed), _Floor, _NoiseOffset, _Frequency, _Amplitude, _Lacunarity, _Persistence, _RidgePower, _RidgeOffset, _Warp);
