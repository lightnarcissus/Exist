...
Shader "Noise/Mobile/Advanced Hermite/Jordan"
{
	Properties 
	{
		...
		_Warp0("Warp 0", Float) = 0.7
		_Warp("Warp", Float) = 0.7
		_Damp0("Damp 0", Float) = 1.0
		_Damp("Damp", Float) = 0.8
	...
	SubShader 
	{
		Pass
		{
			...
			uniform float _Warp0;
			uniform float _Warp;
			uniform float _Damp0;
			uniform float _Damp;
			uniform float _DampScale;
			...
			//
			float fBM(vec3 p, vec3 offset, float frequency, float amplitude, float lacunarity, float persistence, float warp0, float warp, float damp0, float damp, float damp_scale)
			{
				vec4 n = Hermite3D_Deriv(p+offset);
				vec4 n2 = n * n.x;
				float sum = n2.x;
				...
				{
					n = Hermite3D_Deriv((p+offset)*frequency + dsum_warp.xyz);
					n2 = n * n.x;
					sum += damped_amp * n2.x;
					...
					damped_amp = amplitude * (1.0-damp_scale/(1.0+dot(dsum_damp,dsum_damp)));
				}
				return sum;
			}
			...
			{	
				...
				if(_Displace > 0.0)
				{
					position.y = fBM(vec3(position.xz, _Time*_AnimSpeed), _NoiseOffset, _Frequency, _Amplitude, _Lacunarity, _Persistence, _Warp0, _Warp, _Damp0, _Damp, _DampScale);
				}
			...
			void main()
			{
				float h = fBM(vec3(vec2(position.xz), _Time*_AnimSpeed), _NoiseOffset, _Frequency, _Amplitude, _Lacunarity, _Persistence, _Warp0, _Warp, _Damp0, _Damp, _DampScale);
