...
Shader "Noise/Mobile/Advanced Hermite/Badlands" 
{
	Properties 
	{
		...
		_Floor ("Floor", Float) = 1.25
		_RidgePower("Ridge Power", Range(1.0, 8.0)) = 1.2
		_RidgeOffset("Ridge Offset", Float) = 1.15
	...
	SubShader 
	{
		Pass
		{
			...
			float fBM(vec3 p, float floorV, vec3 offset, float frequency, float amplitude, float lacunarity, float persistence, float ridgePower, float ridgeOffset, float warp)
			{
				...
				{
					vec4 n = 0.5 * (0.0 + ridge((p+offset+warp*dsum)*frequency, ridgePower, ridgeOffset));
