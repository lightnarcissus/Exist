...
Shader "Noise/CG/Voronoi/SimplexF13D" 
{
	Properties 
	{
	...
	SubShader 
	{
		Pass
		{
			...
			{
				float h = 0;
				h = SimplexCellular3D((float3(p.x*scale.x+turbulence.x*turbulencePower, p.y*scale.y+turbulence.y*turbulencePower, p.z*scale.z+turbulence.z*turbulencePower)+offset) * frequency);
				h += h*amplitude;
				return h;
			}
