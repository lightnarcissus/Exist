...
Shader "Noise/CG/Hermite/Ridged3D" 
{
	Properties 
	{
	...
	SubShader 
	{
		Pass
		{
			...
			float ridge(float3 p, int powered, float ridgePower, float offset)
			{
				...
				if(powered > 0)
				{
					return pow(offset - abs(4*Hermite3D(p)), ridgePower);
				}
				else
				{
					return offset - abs(4*Hermite3D(p));
				}
			}
			...
			//
			float fBM(float3 p, int octaves, float3 offset, float3 scale, float3 turbulence, float turbulencePower, float frequency, float amplitude, float lacunarity, float persistence, int powered, float ridgePower, float ridgeOffset)
			{
				float sum = 0.0;
				float bias = 0.0;
				for (int i = 0; i < 20; i++)
				{
					if(i >= octaves) break;
					float h = 0.5 * (1 + ridge((float3(p.x*scale.x+turbulence.x*turbulencePower, p.y*scale.y+turbulence.y*turbulencePower, p.z*scale.z+turbulence.z*turbulencePower)+offset)*frequency, powered, ridgePower, ridgeOffset));
					sum += h*amplitude;
					bias -= amplitude;
			...
			float4 frag (v2f i) : COLOR
			{
				...
				float h = fBM(float3(i.worldPos.xyz), _Octaves, _NoiseOffset, _NoiseScale, turbulence, _TurbulencePower, _Frequency, _Amplitude, _Lacunarity, _Persistence, _Powered, _RidgePower, _RidgeOffset);
