...
Shader "Noise/CG/Hermite/Standard" 
{
	Properties 
	{
	...
	SubShader 
	{
		Pass
		{
			...
			float fBM(float3 p, int octaves, float3 offset, float2 scale, float3 turbulence, float turbulencePower, float frequency, float amplitude, float lacunarity, float persistence)
			{
				...
				{
					if(i >= octaves) break;
					float h = Hermite3D((float3(p.x*scale.x+turbulence.x*turbulencePower, p.y*scale.y+turbulence.y*turbulencePower, p.z+turbulence.z*turbulencePower)+offset) * frequency);
					sum += h*amplitude;
