...
Shader "Noise/CG/Other/CubistNoise3D" 
{
	Properties 
	{
	...
	SubShader 
	{
		Pass
		{
			...
			//
			float fBM(float3 p, float3 offset, float3 scale, float3 turbulence, float turbulencePower, float frequency, float amplitude, float2 rangeClamp)
			{
				float h;
				h = Cubist3D((float3(p.x*scale.x+turbulence.x*turbulencePower, p.y*scale.y+turbulence.y*turbulencePower, p.z*scale.z+turbulence.z*turbulencePower)+offset) * frequency, rangeClamp);
				h += h*amplitude;
				return h;
			}
			...
			float4 frag (v2f i) : COLOR
			{
				...
				float h = fBM(float3(i.worldPos.xyz), _NoiseOffset, _NoiseScale, turbulence, _TurbulencePower, _Frequency, _Amplitude, _RangeClamp);
