...
Shader "Noise/CG/Other/PolkaSquare3D" 
{
	Properties 
	{
	...
	SubShader 
	{
		Pass
		{
			...
			//
			float fBM(float3 p, float3 offset, float3 scale, float3 turbulence, float turbulencePower, float frequency, float amplitude, float radiusLow, float radiusHigh)
			{
				float h = 0.0;
				h = PolkaDot3D((float3(p.x*scale.x+turbulence.x*turbulencePower, p.y*scale.y+turbulence.y*turbulencePower, p.z*scale.z+turbulence.z*turbulencePower)+offset) * frequency, radiusLow, radiusHigh);
				h += h*amplitude;
				return h;
			}
			...
			float4 frag (v2f i) : COLOR
			{
				...
				float h = fBM(float3(i.worldPos.xyz), _NoiseOffset, _NoiseScale, turbulence, _TurbulencePower, _Frequency, _Amplitude, _RadiusLow, _RadiusHigh);
