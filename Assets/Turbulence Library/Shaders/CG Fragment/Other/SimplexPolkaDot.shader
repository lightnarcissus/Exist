...
Shader "Noise/CG/Other/SimplexPolkaDot" 
{
	Properties 
	{
	...
	SubShader 
	{
		Pass
		{
			...
			//
			float fBM(float3 p, float3 offset, float2 scale, float3 turbulence, float turbulencePower, float frequency, float amplitude, float radius, float maxDimness)
			{
				float h = 0;
				h = SimplexPolkaDot3D((float3(p.x*scale.x+turbulence.x*turbulencePower, p.y*scale.y+turbulence.y*turbulencePower, p.z+turbulence.z*turbulencePower)+offset) * frequency, radius, maxDimness);
				h += h*amplitude;
				return h;
			}
			...
			v2f vert (appdata_base v)
			{
				...
				if(_Displace > 0.0)
				{
					...
					v.vertex.y = fBM(float3(v.texcoord.xy, _Time*_AnimSpeed), _NoiseOffset, _NoiseScale, turbulence, _TurbulencePower, _Frequency, _Amplitude, _Radius, _MaxDimness);
				}
			...
			float4 frag (v2f i) : COLOR
			{
				...
				float h = fBM(float3(i.texcoord.xy, _Time*_AnimSpeed), _NoiseOffset, _NoiseScale, turbulence, _TurbulencePower, _Frequency, _Amplitude, _Radius, _MaxDimness);
