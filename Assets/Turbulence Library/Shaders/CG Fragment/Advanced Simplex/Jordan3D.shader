...
Shader "Noise/CG/Advanced Simplex/Jordan" 
{
	...
	SubShader 
	{
		Pass
		{
			...
			//
			float fBM(float3 p, int octaves, float3 offset, float3 scale, float3 turbulence, float turbulencePower, float frequency, float amplitude, float lacunarity, float persistence, float warp0, float warp, float damp0, float damp, float damp_scale)
			{
				float4 n = SimplexPerlin3D_Deriv(float3(p.x*scale.x+turbulence.x*turbulencePower, p.y*scale.y+turbulence.y*turbulencePower, p.z*scale.z+turbulence.z*turbulencePower)+offset);
				float4 n2 = n * n.x;
				float sum = n2.x;
				...
				{
					if(i >= octaves) break;
					n = SimplexPerlin3D_Deriv((float3(p.x*scale.x+turbulence.x*turbulencePower, p.y*scale.y+turbulence.y*turbulencePower, p.z*scale.z+turbulence.z*turbulencePower)+offset)*frequency + dsum_warp.xyz);
					n2 = n * n.x;
					sum += damped_amp * n2.x;
			...
			float4 frag (v2f i) : COLOR
			{
				...
				float h = fBM(float3(i.worldPos.xyz), _Octaves, _NoiseOffset, _NoiseScale, turbulence, _TurbulencePower, _Frequency, _Amplitude, _Lacunarity, _Persistence, _Warp0, _Warp, _Damp0, _Damp, _DampScale);
