...
Shader "Noise/CG/Advanced Hermite/Swiss3D" 
{
	Properties 
	{
	...
	SubShader 
	{
		Pass
		{
			...
			//
			float fBM(float3 p, int octaves, float3 offset, float3 scale, float3 turbulence, float turbulencePower, float frequency, float amplitude, float lacunarity, float persistence, int powered, float ridgePower, float ridgeOffset, float warp)
			{
				float sum = 0.0;
				float bias = 0.0;
				...
				{
					if(i >= octaves) break;
					float4 n = 0.5 * (1 + ridge((float3(p.x*scale.x+turbulence.x*turbulencePower, p.y*scale.y+turbulence.y*turbulencePower, p.z*scale.z+turbulence.z*turbulencePower)+offset+warp*dsum)*frequency, powered, ridgePower, ridgeOffset));
					sum += n.x*amplitude;
			...
			float4 frag (v2f i) : COLOR
			{
				...
				float h = fBM(float3(i.worldPos.xyz), _Octaves, _NoiseOffset, _NoiseScale, turbulence, _TurbulencePower, _Frequency, _Amplitude, _Lacunarity, _Persistence, _Powered, _RidgePower, _RidgeOffset, _Warp);
