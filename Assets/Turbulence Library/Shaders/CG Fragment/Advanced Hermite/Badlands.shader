...
Shader "Noise/CG/Advanced Hermite/Badlands" 
{
	Properties 
	{
		...
		_RidgePower("Ridge Power", Range(1.0, 8.0)) = 1.2
	...
	SubShader 
	{
		Pass
		{
			...
			//
			float fBM(float3 p, int octaves, float floorV, float3 offset, float2 scale, float3 turbulence, float turbulencePower, float frequency, float amplitude, float lacunarity, float persistence, int powered, float ridgePower, float ridgeOffset, float warp)
			{
				float sum = floorV;
				float bias = 0.0;
				...
				{
					if(i >= octaves) break;
					float4 n = 0.25 * (0 + ridge((float3(p.x*scale.x+turbulence.x*turbulencePower, p.y*scale.y+turbulence.y*turbulencePower, p.z+turbulence.z*turbulencePower)+offset+warp*dsum)*frequency, powered, ridgePower, ridgeOffset));
			...
			v2f vert (appdata_base v)
			{
				...
				if(_Displace > 0.0)
				{
					...
					v.vertex.y = fBM(float3(v.vertex.xz, _Time*_AnimSpeed), _Octaves, _Floor, _NoiseOffset, _NoiseScale, turbulence, _TurbulencePower, _Frequency, _Amplitude, _Lacunarity, _Persistence, _Powered, _RidgePower, _RidgeOffset, _Warp);
				}
			...
			float4 frag (v2f i) : COLOR
			{
				...
				float h = fBM(float3(i.texcoord.xy, _Time*_AnimSpeed), _Octaves, _Floor, _NoiseOffset, _NoiseScale, turbulence, _TurbulencePower, _Frequency, _Amplitude, _Lacunarity, _Persistence, _Powered, _RidgePower, _RidgeOffset, _Warp);
