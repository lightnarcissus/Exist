...
Shader "Noise/CG/Perlin/Billowed" 
{
	Properties 
	{
	...
	SubShader 
	{
		Pass
		{
			...
			float billow(float3 p, int powered, float billowPower)
			{
				...
				if(powered > 0)
				{
					return pow(abs(4*Perlin3D(p)), billowPower);
				}
				else
				{
					return abs(4*Perlin3D(p));
				}
			}
			...
			v2f vert (appdata_base v)
			{
				...
				if(_Displace > 0.0)
				{
					...
					v.vertex.y = fBM(float3(o.worldPos.xz, _Time*_AnimSpeed), _Octaves, _NoiseOffset, _NoiseScale, turbulence, _TurbulencePower, _Frequency, _Amplitude, _Lacunarity, _Persistence, _Powered, _BillowPower);
				}
				o.worldPos = mul(_Object2World, v.vertex);
			...
			float4 frag (v2f i) : COLOR
			{
				...
				float h = fBM(float3(i.worldPos.xz, _Time*_AnimSpeed), _Octaves, _NoiseOffset, _NoiseScale, turbulence, _TurbulencePower, _Frequency, _Amplitude, _Lacunarity, _Persistence, _Powered, _BillowPower);
