...
Shader "Noise/CG/Advanced Perlin/Jordan3D" 
{
	Properties 
	{
	...
	SubShader 
	{
		Pass
		{
			...
			{
				float4 n = PerlinSurflet3D_Deriv(float3(p.x*scale.x+turbulence.x*turbulencePower, p.y*scale.y+turbulence.y*turbulencePower, p.z*scale.z+turbulence.z*turbulencePower)+offset);
				float4 n2 = n * n.x;
				float sum = n2.x;
				...
				{
					if(i >= octaves) break;
					n = PerlinSurflet3D_Deriv((float3(p.x*scale.x+turbulence.x*turbulencePower, p.y*scale.y+turbulence.y*turbulencePower, p.z*scale.z+turbulence.z*turbulencePower)+offset)*frequency + dsum_warp.xyz);
					n2 = n * n.x;
					sum += damped_amp * n2.x;
