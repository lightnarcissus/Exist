...
Shader "Noise/CG/Advanced Perlin/IQ3D" 
{
	Properties 
	{
	...
	SubShader 
	{
		Pass
		{
			...
			float fBM(float3 p, int octaves, float3 offset, float3 scale, float3 turbulence, float turbulencePower, float frequency, float amplitude, float lacunarity, float persistence)
			{
				...
				{
					if(i >= octaves) break;
					float4 n = PerlinSurflet3D_Deriv((float3(p.x*scale.x+turbulence.x*turbulencePower, p.y*scale.y+turbulence.y*turbulencePower, p.z*scale.z+turbulence.z*turbulencePower)+offset*amplitude) * frequency);
					dsum += n.yzw;
