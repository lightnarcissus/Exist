...
Shader "Noise/CG/Simplex/Billowed" 
{
	Properties 
	{
		...
		_BillowPower("Billow Power", Range(1.0, 8.0)) = 2.0
	}
	...
	SubShader 
	{
		Pass
		{
			...
			float billow(float3 p, int powered, float billowPower)
			{
				// Powered Billow
				if(powered > 0)
				{
					return pow(abs(4*SimplexPerlin3D(p)), billowPower);
				}
				else
				{
					return abs(4*SimplexPerlin3D(p));
				}
			}
			...
			//
			float fBM(float3 p, int octaves, float3 offset, float2 scale, float3 turbulence, float turbulencePower, float frequency, float amplitude, float lacunarity, float persistence, int powered, float billowPower)
			{
				float sum = 0;
				float bias = 0;
				for (int i = 0; i < 20; i++)
				{
					if(i >= octaves) break;
					float h = 0.5 * (1 + billow((float3(p.x*scale.x+turbulence.x*turbulencePower, p.y*scale.y+turbulence.y*turbulencePower, p.z+turbulence.z*turbulencePower)+offset)*frequency, powered, billowPower));
					sum += h*amplitude;
					bias -= amplitude;
			...
			v2f vert (appdata_base v)
			{
				...
				if(_Displace > 0.0)
				{
					...
					v.vertex.y = fBM(float3(v.texcoord.xy, _Time*_AnimSpeed), _Octaves, _NoiseOffset, _NoiseScale, turbulence, _TurbulencePower, _Frequency, _Amplitude, _Lacunarity, _Persistence, _Powered, _BillowPower);
				}
			...
			float4 frag (v2f i) : COLOR
			{
				...
				float h = fBM(float3(i.texcoord.xy, _Time*_AnimSpeed), _Octaves, _NoiseOffset, _NoiseScale, turbulence, _TurbulencePower, _Frequency, _Amplitude, _Lacunarity, _Persistence, _Powered, _BillowPower);
