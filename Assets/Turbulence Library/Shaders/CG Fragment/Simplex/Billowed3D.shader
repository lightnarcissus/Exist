...
Shader "Noise/CG/Simplex/Billowed3D" 
{
	Properties 
	{
	...
	SubShader 
	{
		Pass
		{
			...
			//
			float fBM(float3 p, int octaves, float3 offset, float3 scale, float3 turbulence, float turbulencePower, float frequency, float amplitude, float lacunarity, float persistence, int powered, float billowPower)
			{
				float sum = 0;
				float bias = 0;
				for (int i = 0; i < 20; i++)
				{
					if(i >= octaves) break;
					float h = 0.5 * (1 + billow((float3(p.x*scale.x+turbulence.x*turbulencePower, p.y*scale.y+turbulence.y*turbulencePower, p.z*scale.z+turbulence.z*turbulencePower)+offset)*frequency, powered, billowPower));
					sum += h*amplitude;
					bias -= amplitude;
			...
			float4 frag (v2f i) : COLOR
			{
				...
				float h = fBM(float3(i.worldPos.xyz), _Octaves, _NoiseOffset, _NoiseScale, turbulence, _TurbulencePower, _Frequency, _Amplitude, _Lacunarity, _Persistence, _Powered, _BillowPower);
