...
public class ComboBox
{
    ...
    public int List(Rect rect, string buttonText, GUIContent[] listContent, GUIStyle listStyle, string direction)
    {
        return List(rect, new GUIContent(buttonText), listContent, "button", "box", listStyle, direction);
    }

    public int List(Rect rect, GUIContent buttonContent, GUIContent[] listContent, GUIStyle listStyle, string direction)
    {
        return List(rect, buttonContent, listContent, "button", "box", listStyle, direction);
    }

    public int List(Rect rect, string buttonText, GUIContent[] listContent, GUIStyle buttonStyle, GUIStyle boxStyle, GUIStyle listStyle, string direction)
    {
        return List(rect, new GUIContent(buttonText), listContent, buttonStyle, boxStyle, listStyle, direction);
    }

    public int List(Rect rect, GUIContent buttonContent, GUIContent[] listContent,
                                    GUIStyle buttonStyle, GUIStyle boxStyle, GUIStyle listStyle, string direction)
    {
        if (forceToUnShow)
        {
            forceToUnShow = false;
        ...
        if (isClickedComboButton)
        {
            ...
            if (direction == "down")
            {
                listRect = new Rect(rect.x, rect.y + listStyle.CalcHeight(listContent[0], 1.0f),
